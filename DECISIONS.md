...
|E-039|**6th axis: formality** (verspielt ↔ seriös/formell) — Pentagon becomes a hexagon|A dimension designers steer explicitly; the components are generic over AXES so they follow automatically|
|E-040|**Plain naming** (no jargon/metaphors): paths = **Katalog / Generativ / Persona**; card actions = **Mehr so / Weniger so / Merken**; Lab labels aligned|User directive: precise, sober wording that describes what each thing does — no abstract principle names or fiction|
|E-041|**LLM-judge as the fitness function + judge-select as the master loop.** Eval scores every card 1–5 (coherence/trigger/fit/freshness); Studio now generates OVERSCAN (BATCH+2), judges them on the cheap tier, and keeps the BATCH best (score shown on the card). **Cut x-triad / x-contrast.** Stop tuning the mixer.|Judged eval (84 calls) verdict: the LLM write-step + selection is the real quality lever (+0.94 from render alone, a-bridge 3.19 → a-scene 4.13); the mixing math barely moves quality (Persona, no mix, scored highest at 4.27); the old structural metrics (coherence/uniqueness ≈100% for everything) didn't discriminate — the judge does. x-triad/x-contrast scored 3.04/3.13, below every LLM path, worst fit → removed|